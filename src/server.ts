...
const CRYPTOAPIS_SERVER_INFO = {
    name: "cryptoapis-market-data",
    version: "0.1.0",
    title: "CryptoAPIs Market Data",
...
export async function startMarketDataServer(opts: StartOptions) {
    ...
    if (opts.transport === "stdio") {
        ...
        logger.logInfo("cryptoapis-market-data MCP running (stdio)");
        return;
    }
    ...
    app.listen(port, host, () => {
        logger.logInfo(`cryptoapis-market-data MCP running (http) at http://${host}:${port}${path}`);
